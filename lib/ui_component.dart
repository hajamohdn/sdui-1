...
import 'package:pookie/another_screen.dart';
...
class RowComponent extends UIComponent {
  ...
  factory RowComponent.fromJson(Map<String, dynamic> json) {
    ...
    if (childrenJson is List) {
      ...
      return RowComponent(
        key: json['key'] ?? "",
        padding: double.tryParse(json["data"]["padding"].toString()) ?? 0.0,
        mainAxisAlignment: getMainAlignment(json["data"]["mainAxisAlignment"]),
        children: children,
      );
    } else {
      throw Exception('Expected a list of children in ColumnComponent');
...
class CardComponent extends UIComponent {
  ...
  factory CardComponent.fromJson(Map<String, dynamic> json) {
    return CardComponent(
      key: json['key'],
      padding: double.parse((json["data"]["padding"] ?? 0.0).toString()),
      child: UIComponent.fromJson(json['data']['child']),
